...
전반적으로 예선, 본선에 비해 상당히 낮은 속도로 세팅했고 하이퍼 파라미터와 코드 값은 본선과 동일하게 세팅하였다.
...
오프라인 주행에서 가장 중요한 요소 중 하나는 ```과적합```이다. 과적합은 차량이 특정 트랙에서 과도하게 훈련되어 다른 트랙에서는 제대로 주행하지 못하는 현상을 의미한다.

과적합이 발생한 모델은 온라인 트랙에서는 매우 좋은 기록을 낼 수 있지만, 오프라인 트랙에서는 기대에 미치지 못하는 결과를 보인다. 이는 오프라인 트랙이 경우에 따라 축소되거나, 온라인 트랙과 완벽히 일치하지 않기 때문이다.

이를 해결하기 위해, 나는 보상함수 그래프에 더욱 신경을 써서 모델을 훈련시켰다. 특히, 모델의 완주율이 100%에 근접하면 훈련을 멈추는 방식으로 과적합을 방지했다.
...
결승 주행에서 사용한 차량이다.  
<img src="https://github.com/user-attachments/assets/e18d53ba-bab4-4c82-932f-480201685517" width="320" height="470"/>

1차 테스트 주행에서는 최적 경로 모델로 시작했지만, 커브 구간에서 자주 이탈하며 오프라인 트랙에서는 적합하지 않은 모습을 보였다.

이러한 상황을 대비해 준비한 중앙선 모델로 2차 테스트 주행을 진행했다. 

중앙선 모델은 이전 모델보다 훨씬 안정적이었고, 완주율도 높게 나타났다. 비록 최적 경로 모델보다 기록은 느리고 훈련도 간편했지만, 오프라인 트랙에서는 훨씬 좋은 성능을 보여주었다. 

이 과정을 통해 온라인과 오프라인 트랙 간의 차이를 뚜렷하게 느낄 수 있었다.

최종적으로, slow3-3 모델이 오프라인 주행에서 최고 기록을 세웠다. 아래는 그 주행 영상이다.
...
최종적으로 ```12.653```의 랩타임으로 전체 2등으로 결승 주행을 마무리 지었다.

## 대회 전체 소감
AWS 딥레이서 대회는 나에게 깊은 의미가 있는 도전이었다. 총 4번의 도전 끝에, 수많은 시행착오를 거쳐 좋은 성과를 얻을 수 있었다.

특히 춘천에서 열린 이번 대회는 특별했다. 각 팀에게 차량을 한 대씩 지원해 주었고, 하드웨어적인 문제 없이 경기에 집중할 수 있었다.  
대회 주최 측의 지원과 교육 수준도 매우 뛰어나, 참가자들이 연구에 온전히 몰입할 수 있는 환경이 조성되었다.

이번 도전을 마지막으로 딥레이서 대회에 더 이상 참가하지 않을 것 같다. 그러나 나의 힘으로 유의미한 결과를 만들어낸 경험은 매우 값지며, 이 대회를 성공적으로 마무리 지을 수 있어 뿌듯하다.
